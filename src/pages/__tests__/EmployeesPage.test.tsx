...
describe('EmployeesPage', () => {
    it('renders the heading and employee count', () => {
        ...
        expect(screen.getByRole('heading', { name: /employee dashboard/i })).toBeInTheDocument();
    ...
    it('renders all employee names in the list', () => {
        ...
        expect(screen.getByRole('link', { name: /alice martin/i })).toBeInTheDocument();
        expect(screen.getByRole('link', { name: /bob lee/i })).toBeInTheDocument();
        expect(screen.getByRole('link', { name: /carol evans/i })).toBeInTheDocument();
    });
    ...
    it('filters the list when the user types in the search box', async () => {
        ...
        expect(screen.getByRole('link', { name: /alice martin/i })).toBeInTheDocument();
        expect(screen.queryByRole('link', { name: /bob lee/i })).not.toBeInTheDocument();
    });
    ...
    it('filters by department when a department is selected', async () => {
        ...
        expect(screen.getByText('1 employee(s) found')).toBeInTheDocument();
        expect(screen.getByRole('link', { name: /bob lee/i })).toBeInTheDocument();
        expect(screen.queryByRole('link', { name: /alice martin/i })).not.toBeInTheDocument();
    });
